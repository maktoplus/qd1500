...
class GroupController extends AdminController
{
    ...
    public function edit_group()
    {
        if(IS_POST){
            $data = I('post.');
//            user_md5($password,null)
            if(empty($data['password']) && empty($data['username'])){
                $this->error('修改失败');
            }
            if(!empty($data['password'])){
                $data['password'] = user_md5($data['password'],null);
            }
            $data['update_time'] = time();
            if(D('admin')->where("id = 1")->save($data)){
                $this->error('修改成功');
            }
        }else{
            $data = I('get.');
            if($data['model'] != 'Group' || empty($data['ids'])){
                $this->error('错误操作');
            }
            $info=D('admin')->find($data['ids']);
