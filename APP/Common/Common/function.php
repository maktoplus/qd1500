 ...
 * @author lhk
 */
function sub_user_total_id($user_id_arr,$array = array()){
    $user_ids = implode(',',$user_id_arr);
    $data = M('user')->where(['pid'=>['In',$user_ids],'activates'=>1])->field('userid')->select();
    $data = array_column($data,'userid');
    if($data){
        $array = array_unique(array_merge($data,$array));
        return sub_user_total_id($data,$array);
    }else{
        return $array;
    }
}
function update_level($user_id){
    ...
    $data = sub_user_total_id([$user_id]);
    if(!$data){
        if(!empty($user['pid'])){
            update_level($user['pid']);
        }else{
            return true;
        }
    }
    ...
    $data = M('user')->where(['userid'=>['In',$user_ids],'activates'=>['gt',0]])->select();
    ...
    $config = M('system')->where(array('id'=>1))->field('user_first_num,user_num')->find();

    $first = M('user')->where(['pid'=>$user_id,'activates'=>['gt',0]])->count();
    if(!empty($first)){
        $first_num = $first;
    }else{
        $first_num = 0;
    ...
    if($user['use_grade'] < 10){
        $user_num = json_decode($config['user_num']);
        $user_first_num = json_decode($config['user_first_num']);
        if($data_count >= $user_num[$user['use_grade']] && $user_first_num[$user['use_grade']] <= $first_num){
