...
function send_sms($mobile,$content){
    ...
    $res = curlPost("http://api.eait.cn/Sms/SmsHttp_U.aspx");
...
function order_rand()
{
    ...
    $d = rand('1000','999999999');
    if(strlen($d) == 4){
        $d = GetRandStr(2).$d;
    }elseif(strlen($d) == 5){
        $d = GetRandStr(1).$d;
    }
    if(M('recharge')->where("orderid = {$d}")->count()){
        order_rand();
    }else{
        return $d;
    }
...
function for_user($user_id,$num = 0)
{
    if($num == 0){
        $arr = [];
        $num = 100;
        for($i = 0;$i<$num;$i++){
            $user = M("user")->where("pid in($user_id)")->field('userid')->select();
            $arr[$i] = $user_id;
            if(empty($user)){
                return implode($arr);
            }
